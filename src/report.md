...
- Скачать установочный образ ОС https://old-releases.ubuntu.com/releases/20.04/ (Установочный образ Ubuntu (ISO-образ) — это специальный файл, содержащий полную копию файловой системы, необходимую для установки или запуска Ubuntu)
- Скачать программу для виртуализации https://www.virtualbox.org/wiki/Downloads (Oracle VM VirtualBox — это бесплатная платформа виртуализации с открытым исходным кодом, которая позволяет запускать несколько операционных систем на одном компьютере одновременно)
- Создание виртуальной машины (Виртуальная машина (Virtual Machine, VM) — это программная эмуляция физического компьютера, которая работает внутри вашей основной операционной системысв )
- Установка ОС
- Узнать версию Ubuntu, выполнив команду `cat /etc/issue.`

![установленная ОС в VirtualBox](../misc/images/1task_oracle.png)

Установленная ОС в VirtualBox
...
Создаем нового пользователя в группе adm

  - `sudo useradd -m -G adm -s /bin/bash titaniya`

>`useradd` Основная команда для создания новых пользователей в Linux

 >`-m` (--create-home) Создать домашний каталог пользователя, без этого флага домашняя директория не создается

>`-G adm` (--groups) Добавить пользователя в дополнительные группы

>`adm` — группа административного мониторинга

Пользователь будет состоять в двух группах: основная группа: titaniya (автоматически создается),дополнительная: adm

>`-s /bin/bash` (--shell) Установить командную оболочку (shell)

/bin/bash — Bourne Again SHell (стандартная оболочка), без этого флага используется системный shell по умолчанию (обычно /bin/sh)
...
Используя команду ps, показываем наличие процесса sshd. Для этого к команде нужно подобрать ключи.
Значение команды и каждого ключа в ней описаны ниже.
